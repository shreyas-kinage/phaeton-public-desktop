...
function VoteModal(props) {
  ...
  const checkAmount = async (event) => {
    ...
    if (amounts > details?.token?.balance / Math.pow(10, 8) || amounts == 0) {
      setDefaultState(true);
      setAmountWarning(true);
      return;
    }
  ...
  const calcProcessingFee = async (amounts) => {
    let transactionObject = {
      ...
      moduleAssetId: "5:1",
      votes: [{ amount: BigInt(amounts), delegateAddress: delegateAddress }],
    };
    ...
    new Promise(async (resolve, reject) => {
      try {
        let transactionObject = {
          ...
          username: decryptedData.username,
          moduleAssetId: "5:1",
          votes: [{ amount: BigInt(amount), delegateAddress: delegateAddress }],
        };
        setIsLoading(true);
        const { moduleAssetId, ...rawTransaction } = transactionObject;
    ...
    <>
      ...
      >
        <Modal.Header className="modalheader" >
          <Modal.Title className="receivetext mb-0 pb-0">Vote
        ...
        <Modal.Body className="modalBody vote-bg pt-0 mt-0">
