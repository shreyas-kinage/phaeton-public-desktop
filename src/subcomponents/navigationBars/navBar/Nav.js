...
import ToggleButton from "@tools/Button/ToggleButton";
...
const Nav = ({ accountDetails, blockDetails }) => {
      ...
      <div className={`${styles.navbar} sticky-top`}>
        ...
        <div ref={clickNode} className={slider ? `${styles.navbarnew}` : `${styles.navbarnewactive}`}>
          ...
          <ul className={`${styles.uldesign}`}>
            ...
            <div className={showEx ? `${styles.fadeInText}` : `d-none ${styles.fadeInText}`}>
        ...
        {accountDetails || blockDetails ? "" : <ToggleButton isLoading={isLoading} setIsLoading={setIsLoading} />}
      </div>
    </>
  );
};
