 ...
 */
const VoteItem = ({ vote, address, title, truncate }) => {
    ...
    <span className={`${styles.container} vote-item-address`}>
      ...
      <span className={styles.value}>
        {vote.confirmed && vote.unconfirmed ? (
          <>
            <PhaeAmount val={vote.confirmed} token={token} />
            <span className={styles.arrowIcon}>➞</span>
            <PhaeAmount val={vote.unconfirmed} token={token} />
          </>
        ) : (
          <PhaeAmount val={Object.values(vote)[0]} token={token} />
        )}
      </span>
    </span>
  );
