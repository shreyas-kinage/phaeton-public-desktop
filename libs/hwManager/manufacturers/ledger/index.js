...
import { getBufferToHex, getTransactionBytes } from "./utils";
 ...
 * the user is inside the PHAE App, if not then will show the device as connected but not
...
const getPublicKey = async (transporter, { device, data }) => {
  ...
  try {
    ...
    const { publicKey: res } = await liskLedger.getPubKey(
      ledgerAccount,
      data.showOnDevice
    );
    transport.close();
